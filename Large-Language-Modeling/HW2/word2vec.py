...
def naiveSoftmaxLossAndGradient(
    ...
    gradCenterVec = np.dot(prob, outsideVectors) - outsideVectors[outsideWordIdx]
...
def negSamplingLossAndGradient(
    ...
    posWord = sigmoid(score[0])
    negWords = sigmoid(-score[1:])

    loss = -np.log(posWord) - np.sum(np.log(negWords))
    gradCenterVec = np.dot(posWord - 1, outsideVectors[outsideWordIdx])
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    gradOutsideVecs[outsideWordIdx] = (posWord - 1) * centerWordVec

    for i, idx in enumerate(negSampleWordIndices):
        gradCenterVec +=  np.dot(1 - negWords[i], outsideVectors[idx])
        gradOutsideVecs[idx] += (1 - negWords[i]) * centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for word in outsideWords:
        ...
        newLoss, newGradCenterVecs, newGradOutsideVectors = word2vecLossAndGradient(centerWordVectors[centerIdx], word2Ind[word], outsideVectors, dataset)
        loss += newLoss
        gradCenterVecs[centerIdx] += newGradCenterVecs
        gradOutsideVectors += newGradOutsideVectors
